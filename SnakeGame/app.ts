...
// Snake properties
const snake: { x: number; y: number }[] = [{ x: 200, y: 200 }];
let dx = 20;
let dy = 0;
let food = { x: 0, y: 0 };
...
let bigFoodTimer: number | null = null; // Timer for big food
...
function changeDirection(event: KeyboardEvent) {
  if (isPaused || !gameRunning) return; // Don't move the snake while paused or before starting
...
function togglePause() {
  if (!gameRunning) return; // Can't pause if the game hasn't started
  isPaused = !isPaused;
...
function resetGame() {
  ...
  if (bigFoodTimer) clearTimeout(bigFoodTimer); // Clear any previous big food timers
}
